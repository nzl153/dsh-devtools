...
*/
const HOME = process.env.DSH_HOME ?? (process.platform === "win32" ? process.env.USERPROFILE ?? process.env.HOME ?? "." : process.env.HOME ?? ".");
function service() {
...
function parseBranchJson(v, fallbackId) {
	if (typeof v !== "string" || v.length === 0) return null;
	try {
		const o = JSON.parse(v);
		return {
			id: o["id"] === "a" || o["id"] === "b" ? o["id"] : fallbackId,
			...
			evaluator: o["evaluator"] && typeof o["evaluator"] === "object" ? o["evaluator"] : void 0
		};
	} catch (error) {
...
async function main() {
	...
	switch (cmd) {
		case "create": {
			...
			printExperiment(await svc.create(input));
			return;
		}
		case "list": {
			const exps = await svc.list();
			console.log(JSON.stringify(exps.map((e) => ({
				id: e.id,
				title: e.title,
				status: e.status,
		...
		case "run": {
			...
			if (exp.result) {
				...
				for (const run of exp.result.runs) {
					const s = run.summary;
					console.log(`  ${run.branch} x${run.repeat}: successRate=${(s.successRate * 100).toFixed(0)}% medianWall=${s.medianWallTimeMs ?? "n/a"}ms medianToolCalls=${s.medianToolCalls ?? "n/a"} medianTokens=${s.medianTokens ?? "n/a"}`);
				}
			}
			printExperiment(exp);
			return;
