// 构建产物守卫：host / cli / client 三个 half 都产出。
import fs from 'node:fs'

const EXPECTED = [
  { file: 'lib/index.js', min: 3000 },
  { file: 'lib/cli.js', min: 3000 },
...
if (fs.existsSync('lib/cli.js')) {
  const head = fs.readFileSync('lib/cli.js', 'utf8').slice(0, 40)
  console.log(`${head.includes('#!/usr/bin/env node') ? 'PASS' : 'FAIL'}  cli.js 保留 shebang`)
  if (!head.includes('#!/usr/bin/env node')) failures++
}
