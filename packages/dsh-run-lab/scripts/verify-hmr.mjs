// verify-hmr：校验当前插件仓库与运行中 DSH 的 HMR 链路一致（profile link + node_modules realpath + graph rev）。
...
check('client bundle 存在', hasClient, hasClient ? clientPath : '缺少 lib/client.js')
...
if (existsSync(profilePkgPath)) {
  ...
  const rootNorm = norm(root).replace(/\\/g, '/').toLowerCase()
  if (typeof dep === 'string' && dep.startsWith('link:')) {
    const linkTarget = norm(dep.slice(5)).replace(/\\/g, '/').toLowerCase()
    linkOk = linkTarget === rootNorm
...
try {
  const actual = realpathSync(nmLink).replace(/\\/g, '/').toLowerCase()
  realOk = actual === root.replace(/\\/g, '/').toLowerCase()
