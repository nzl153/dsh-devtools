...
// The bundle is CJS: it references `require` in the factory scope.
vm.runInContext(code, window, { filename: 'client.js' })
check('bundle registers __ModuleLoader__', handoff !== null && handoff.id === 'dsh-context-xray')
...
// Mock client context enough for apply() to mount.
const registrations = []
const ctx = {
  locale: {
    register: () => () => {},
